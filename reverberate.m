%REVERBERATE Digital Moorer Reverb Implementation
% This function simulates reverberation based on the Moorer reverb
% algorithm, using tapped delay lines for early reflections and parallel
% comb filters for the diffuse reverb sound.
% 
% Input Variables:
%   input : input signal
%   mix : balance between direct (dry) sound and the reverberated signal
%   feedback : feedback gain used in the comb filters
%   reflectionPresetPath : early reflections delay times preset file path
%   Fs : sampling frequency
function [output, ir] = reverberate(input, Fs, mix, feedback, reflectionPresetPath)

    % Load the early reflections delay times and delay gains from a CSV file
    ...
    for n=1:nIrSamples
        ...
        % Sum the output of the comb filters
        combOutput = 0.25 * irComb1 + irComb2 + irComb3 + irComb4;

        % All pass filter output of comb filters
    ...
    for n = 1:nOutputSamples
        ...
        % All pass filter the output of the comb filters
